...
function buildOrderReceipt(tableName, order, orderIndex, tableTotal) {
  const line  = "--------------------------------\n";
  const dline = "================================\n";
  ...
  data = concat(data,
    txt(dline),
    txt(pad("SUBTOTAL", "R$" + subtotal.toFixed(2))),
    txt(line),
    esc(0x61, 0x01),
    txt("Obrigado pela preferencia!\n"),
    txt("Bom apetite! 🌴\n"),
    txt("\n\n\n"),
    gs(0x56, 0x41, 0x10), // cut
  );
  return data;
}
...
function buildTableReceipt(table) {
  ...
  data = concat(data,
    txt(dline),
    txt(pad("TOTAL", "R$" + total.toFixed(2))),
    txt(line),
    esc(0x61, 0x01),
    txt("Obrigado pela preferencia!\n"),
...
function PrintButton({ label, buildData, small }) {
  const [status, setStatus] = useState("idle"); // idle | printing | ok | err
  ...
  return (
    <div>
      <button onClick={handle} disabled={status==="printing"} style={{
        ...
        cursor: status==="printing" ? "not-allowed" : "pointer",
      ...
      {status==="err" && (
        <div style={{ fontSize:11, color:"#fca5a5", marginTop:4, maxWidth:200 }}>{errMsg}</div>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const sendOrder = () => {
    ...
    const updater = t => t.id===selectedTable.id ? {...t, orders:[...t.orders, newOrder]} : t;
    setTables(p => p.map(updater));
  ...
  const markDelivered = (tableId, orderId, itemId) => {
    const updater = t => {
      ...
      return { ...t, orders: t.orders.map(o => {
        if (o.id!==orderId) return o;
        return { ...o, items: o.items.map(it =>
          it.id===itemId ? {...it, delivered:Math.min(it.delivered+1, it.qty)} : it
        )};
      })};
    };
    setTables(p => p.map(updater));
    if (selectedTable?.id===tableId) setSelectedTable(p => updater(p));
  };
  ...
  const allPendingItems = tables.filter(t=>t.status==="open").flatMap(t =>
    t.orders.flatMap(o => o.items.filter(it=>it.delivered<it.qty).map(it => ({
      ...it, tableName:t.name, tableId:t.id, orderId:o.id, pending:it.qty-it.delivered
    })))
  );
  ...
  return (
    <div style={{ fontFamily:"'Nunito',sans-serif", minHeight:"100vh", background:"#0f172a", color:"#f1f5f9" }}>
      <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&display=swap" rel="stylesheet"/>

      {/* Header */}
      <header style={{
        background:"linear-gradient(135deg,#1e3a5f,#0f2744)", padding:"16px 20px",
        display:"flex", alignItems:"center", justifyContent:"space-between",
        boxShadow:"0 4px 20px rgba(30,58,95,.6)", position:"sticky", top:0, zIndex:100,
      }}>
        <div>
          <div style={{fontSize:22,fontWeight:900}}>🌴 Nosso Quintal</div>
          <div style={{fontSize:12,opacity:.85,fontWeight:600}}>
            {openTables.length} mesa{openTables.length!==1?"s":""} abertas • {allPendingItems.length} itens pendentes
          </div>
        </div>
        {activeTab==="order" && selectedTable && (
          <button onClick={()=>{setActiveTab("tables");setSelectedTable(null);setCart([]);}} style={{
            background:"rgba(0,0,0,.2)",border:"none",color:"#fff",padding:"8px 14px",
      ...
      {/* Bottom Nav */}
      {activeTab!=="order" && (
        <nav style={{position:"fixed",bottom:0,left:0,right:0,background:"#1e293b",display:"flex",borderTop:"1px solid #334155",zIndex:100}}>
          {[{key:"tables",label:"Mesas",emoji:"🪑"},{key:"pending",label:`Pendentes${allPendingItems.length>0?` (${allPendingItems.length})`:""}`,emoji:"⏳"}].map(tab=>(
            <button key={tab.key} onClick={()=>setActiveTab(tab.key)} style={{
              ...
              color:activeTab===tab.key?"#3b82f6":"#64748b",fontWeight:800,fontSize:13,cursor:"pointer",
              fontFamily:"inherit",borderTop:activeTab===tab.key?"3px solid #3b82f6":"3px solid transparent",
        ...
        <div style={{padding:"20px 16px",paddingBottom:80}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
            ...
            <button onClick={()=>setShowNewTable(true)} style={{
              background:"linear-gradient(135deg,#3b82f6,#1e3a5f)",border:"none",color:"#fff",
              padding:"10px 18px",borderRadius:12,fontWeight:800,fontSize:14,cursor:"pointer",
              fontFamily:"inherit",boxShadow:"0 4px 12px rgba(59,130,246,.4)",
            }}>+ Nova Mesa</button>
          </div>

          {showNewTable && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
              <div style={{background:"#1e293b",borderRadius:20,padding:28,width:"85%",maxWidth:360}}>
                ...
                <input autoFocus value={newTableName} onChange={e=>setNewTableName(e.target.value)}
                  ...
                  style={{width:"100%",padding:"12px 14px",borderRadius:10,background:"#0f172a",
                    border:"2px solid #334155",color:"#f1f5f9",fontSize:16,fontFamily:"inherit",
                    fontWeight:700,boxSizing:"border-box",marginBottom:16}}/>
          ...
          <div style={{display:"grid",gap:12}}>
            {openTables.map(table => {
              const pending = tablePendingCount(table);
              const total   = tableTotal(table);
              return (
                <div key={table.id} style={{background:"#1e293b",borderRadius:16,padding:16,border:`2px solid ${pending>0?"#3b82f6":"#22c55e"}`,cursor:"pointer"}}
            ...
            <>
              ...
              <div style={{display:"grid",gap:8}}>
                {closedTables.map(table=>(
                  <div key={table.id} style={{background:"#1e293b",borderRadius:12,padding:"12px 16px",border:"2px solid #334155",opacity:.6,display:"flex",justifyContent:"space-between"}}>
                    <div>
                      <div style={{fontWeight:800}}>{table.name}</div>
                      <div style={{fontSize:12,color:"#64748b"}}>Fechada às {fmt(table.closedAt)}</div>
                    </div>
                    <div style={{fontWeight:800,color:"#64748b"}}>R$ {tableTotal(table).toFixed(2)}</div>
                  </div>
                ))}
              </div>
            </>
            ...
            {allPendingItems.map(item=>(
              <div key={`${item.orderId}-${item.id}`} style={{background:"#1e293b",borderRadius:14,padding:"14px 16px",border:"2px solid #3b82f6",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                  <div style={{background:"#3b82f6",color:"#000",borderRadius:20,padding:"2px 12px",fontWeight:900,fontSize:14,minWidth:32,textAlign:"center"}}>{item.pending}x</div>
        ...
        <div style={{paddingBottom:140}}>
          ...
          <div style={{background:"#1e293b",padding:"16px 20px",borderBottom:"1px solid #334155",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {/* Print full table account */}
              {selectedTable.orders.length > 0 && (
                <PrintButton
                  label="Conta"
                  buildData={() => buildTableReceipt(selectedTable)}
                />
              )}
                  ...
                  <div key={order.id} style={{background:"#1e293b",borderRadius:14,padding:14,marginBottom:12,border:"1px solid #334155"}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <div style={{fontSize:12,color:"#64748b",fontWeight:700}}>
                        Pedido #{oi+1} • {fmt(order.createdAt)}
                      </div>
                      <PrintButton
                        small
                        label={`Pedido #${oi+1}`}
                        buildData={() => buildOrderReceipt(selectedTable.name, order, oi, tableTotal(selectedTable))}
                      />
                    </div>

                    {order.items.map(item=>{
                      const allDelivered = item.delivered>=item.qty;
                      return (
                        <div key={item.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #0f172a"}}>
                          <div style={{display:"flex",alignItems:"center",gap:10}}>
                            ...
                            <div>
                              <div style={{fontWeight:700,fontSize:14,textDecoration:allDelivered?"line-through":"none",color:allDelivered?"#475569":"#f1f5f9"}}>{item.qty}x {item.name}</div>
                              <div style={{fontSize:12}}>
                                {allDelivered
                                  ? <span style={{color:"#22c55e",fontWeight:700}}>✓ Entregue</span>
                                  : <span style={{color:"#3b82f6",fontWeight:700}}>⏳ {item.delivered}/{item.qty} entregue{item.delivered!==1?"s":""}</span>}
                              </div>
                            </div>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            ...
                            {!allDelivered && (
                              <button onClick={()=>markDelivered(selectedTable.id,order.id,item.id)} style={{background:"#22c55e",border:"none",color:"#fff",borderRadius:8,padding:"4px 10px",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>✓</button>
                            )}
                          </div>
                        </div>
                      );
                    })}
                    <div style={{marginTop:8,textAlign:"right",fontWeight:900,color:"#3b82f6"}}>
                      R$ {orderSubtotal.toFixed(2)}
                    </div>
                  </div>
                );
              })}

              {/* Total + print full account */}
            ...
            <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8,marginBottom:12}}>
              {categories.map(cat=>(
                <button key={cat} onClick={()=>setFilterCategory(cat)} style={{
                  ...
                  background:filterCategory===cat?"#3b82f6":"#1e293b",
          ...
          {cart.length>0 && (
            <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#1e293b",borderTop:"2px solid #3b82f6",padding:"12px 16px",zIndex:100}}>
              <div style={{marginBottom:10}}>
                {cart.map(c=>(
                  <div key={c.itemId} style={{display:"flex",justifyContent:"space-between",fontSize:13,color:"#94a3b8",marginBottom:4}}>
                    <span>{c.emoji} {c.qty}x {c.name}</span>
              ...
              <button onClick={sendOrder} style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#3b82f6,#1e3a5f)",border:"none",color:"#fff",borderRadius:14,fontWeight:900,fontSize:16,cursor:"pointer",fontFamily:"inherit",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span>🛍️ Enviar Pedido ({cartCount} itens)</span>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
          <div style={{background:"#1e293b",borderRadius:20,padding:28,width:"85%",maxWidth:360}}>
            ...
            <div style={{fontSize:14,color:"#94a3b8",marginBottom:20}}>
              Total: R$ {tableTotal(tables.find(t=>t.id===confirmClose)||{orders:[]}).toFixed(2)}
            </div>
            <div style={{display:"flex",gap:10}}>
